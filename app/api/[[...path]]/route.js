  ...
  try {
    ...
    if (route === "/auth/login" && method === "POST") {
      ...
      if (needsMigration) {
        ...
        await db
          ...
          .updateOne({ id: calisan.id }, { $set: { sifre: hashedPassword } });
      }
      ...
      // Get department name
      const departman = await db
        .collection("departmanlar")
        .findOne({ id: calisan.departmanId });
    ...
    if (route === "/dashboard" && method === "GET") {
      ...
      const enrichedZimmetler = await Promise.all(
        zimmetler.map(async (zimmet) => {
          const envanter = await db
            .collection("envanterler")
            .findOne({ id: zimmet.envanterId });
          const calisan = await db
            .collection("calisanlar")
            .findOne({ id: zimmet.calisanId });
          const departman = calisan
            ? await db
                .collection("departmanlar")
                .findOne({ id: calisan.departmanId })
            : null;
          const envanterTip = envanter
            ? await db
          ...
          return {
            id: zimmet.id,
            envanterAd: envanter
              ? `${envanterTip?.ad || ""} ${envanter.marka} ${envanter.model}`
              : "Bilinmiyor",
      ...
      const stats = {
        ...
        arizali: envanterler.filter(
          (e) => e.durum === "Arızalı" || e.durum === "Kayıp",
        ).length,
      };
      ...
      const servistekiCihazlar = await db
      ...
      const finishedRepairs = await db
        .collection("bakim_kayitlari")
        .find({
          durum: "Tamamlandı",
          deletedAt: null,
    ...
    if (route === "/departmanlar" && method === "GET") {
      ...
      return handleCORS(
        NextResponse.json(departmanlar.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route === "/departmanlar" && method === "POST") {
      ...
      const existing = await db
        .collection("departmanlar")
        .findOne({ ad: body.ad, deletedAt: null });
      if (existing) {
        return handleCORS(
          NextResponse.json(
    ...
    if (route.startsWith("/departmanlar/") && method === "PUT") {
      ...
      if (result.matchedCount === 0) {
        return handleCORS(
          NextResponse.json({ error: "Departman bulunamadı" }, { status: 404 }),
        );
      }
      ...
      if (
        updateFields.aciklama !== undefined &&
        updateFields.aciklama !== existingDept?.aciklama
      ) {
        changedFields.aciklama = {
          onceki: existingDept?.aciklama || "",
          yeni: updateFields.aciklama,
        };
      }
    ...
    if (route === "/calisanlar" && method === "GET") {
      ...
      const enrichedCalisanlar = await Promise.all(
        calisanlar.map(async (calisan) => {
          const departman = await db
            .collection("departmanlar")
            .findOne({ id: calisan.departmanId });
          const calisanZimmetleri = aktifZimmetler.filter(
            (z) => z.calisanId === calisan.id,
          );
          return {
            ...calisan,
            departmanAd: departman?.ad || "Bilinmiyor",
      ...
      return handleCORS(
        NextResponse.json(enrichedCalisanlar.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route === "/calisanlar" && method === "POST") {
      ...
      const calisan = {
        ...
        iseGirisTarihi:
          body.iseGirisTarihi || new Date().toISOString().split("T")[0],
      ...
      // Audit log
      const requestingUserForLog = authHeader
        ? await db.collection("calisanlar").findOne({ id: authHeader })
        : null;
      await createAuditLog(
    ...
    if (route.match(/\/calisanlar\/[^/]+$/) && method === "GET") {
      ...
      if (!calisan) {
        return handleCORS(
          NextResponse.json({ error: "Çalışan bulunamadı" }, { status: 404 }),
        );
      }
    ...
    if (route.startsWith("/calisanlar/") && method === "PUT") {
      ...
      const authHeader = request.headers.get("x-user-id");
      const existingCalisan = await db
        .collection("calisanlar")
        .findOne({ id, deletedAt: null });

      if (
        (body.adminYetkisi !== undefined ||
          body.yoneticiYetkisi !== undefined) &&
        authHeader
      ) {
        if (
          existingCalisan &&
        ...
        if (activeZimmet) {
          return handleCORS(
            NextResponse.json(
              {
                error:
                  "Üzerinde aktif zimmet olan çalışan pasife alınamaz. Önce zimmetleri iade alın.",
              },
              { status: 409 },
            ),
          );
      ...
      const changedFields = {};
      if (
        updateFields.adSoyad &&
        updateFields.adSoyad !== existingCalisan?.adSoyad
      ) {
        changedFields.adSoyad = {
          onceki: existingCalisan?.adSoyad,
          yeni: updateFields.adSoyad,
        };
      }
      if (updateFields.email && updateFields.email !== existingCalisan?.email) {
        changedFields.email = {
          onceki: existingCalisan?.email,
          yeni: updateFields.email,
        };
      }
      if (
        updateFields.telefon !== undefined &&
        updateFields.telefon !== existingCalisan?.telefon
      ) {
        changedFields.telefon = {
          onceki: existingCalisan?.telefon || "",
          yeni: updateFields.telefon,
        };
      }
      if (
        updateFields.departmanId &&
        updateFields.departmanId !== existingCalisan?.departmanId
      ) {
        changedFields.departmanId = {
          onceki: existingCalisan?.departmanId,
          yeni: updateFields.departmanId,
        };
      }
      if (updateFields.durum && updateFields.durum !== existingCalisan?.durum) {
        changedFields.durum = {
          onceki: existingCalisan?.durum,
          yeni: updateFields.durum,
        };
      }
      if (
        updateFields.yoneticiYetkisi !== undefined &&
        updateFields.yoneticiYetkisi !== existingCalisan?.yoneticiYetkisi
      ) {
        changedFields.yoneticiYetkisi = {
          onceki: existingCalisan?.yoneticiYetkisi,
          yeni: updateFields.yoneticiYetkisi,
        };
      }
      if (
        updateFields.adminYetkisi !== undefined &&
        updateFields.adminYetkisi !== existingCalisan?.adminYetkisi
      ) {
        changedFields.adminYetkisi = {
          onceki: existingCalisan?.adminYetkisi,
          yeni: updateFields.adminYetkisi,
        };
      }
    ...
    if (
      ...
      try {
        currentMatch = await bcrypt.compare(
    ...
    if (
      ...
      return handleCORS(
        NextResponse.json(enrichedZimmetler.map(({ _id, ...rest }) => rest)),
      );
    }

    if (
      route.startsWith("/calisanlar/") &&
      route.endsWith("/belgeler") &&
      method === "GET"
    ) {
      const id = route.split("/")[2];

      const belgeler = await db
        .collection("calisan_belgeleri")
        .find({ calisanId: id, deletedAt: null })
        .sort({ createdAt: -1 })
        .toArray();

      return handleCORS(
        NextResponse.json(belgeler.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route.startsWith("/calisanlar/belgeler/") && method === "DELETE") {
      ...
      if (result.matchedCount === 0) {
        return handleCORS(
          NextResponse.json({ error: "Belge bulunamadı" }, { status: 404 }),
        );
      }
    ...
    if (
      ...
      const calisan = await db.collection("calisanlar").findOne({ id });
      await createAuditLog(
        body.userId || "system",
        body.userName || "System",
        "PASSWORD_RESET",
      ...
      if (activeZimmet) {
        return handleCORS(
          NextResponse.json(
            {
              error:
                "Bu çalışanın üzerinde aktif zimmet var, silinemez. Önce zimmetleri iade alın.",
            },
            { status: 409 },
          ),
        );
    ...
    if (route === "/envanter-tipleri" && method === "GET") {
      ...
      const enrichedTipler = await Promise.all(
        tipler.map(async (tip) => {
          ...
          return {
            ...
            zimmetliSayisi: envanterler.filter((e) => e.durum === "Zimmetli")
              .length,
            depodaSayisi: envanterler.filter((e) => e.durum === "Depoda")
              .length,
            arizaliSayisi: envanterler.filter(
              (e) => e.durum === "Arızalı" || e.durum === "Kayıp",
            ).length,
          };
        }),
      ...
      return handleCORS(
        NextResponse.json(enrichedTipler.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route === "/envanterler" && method === "GET") {
      ...
      const enrichedEnvanterler = await Promise.all(
        envanterler.map(async (envanter) => {
          ...
          if (activeZimmet) {
            const calisan = await db
              .collection("calisanlar")
              .findOne({ id: activeZimmet.calisanId });
            zimmetBilgisi = {
              calisanAd: calisan?.adSoyad || "Bilinmiyor",
      ...
      return handleCORS(
        NextResponse.json(enrichedEnvanterler.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route === "/envanterler" && method === "POST") {
      ...
      // Run audit log and email notification in background (fire-and-forget)
      (async () => {
        try {
          await createAuditLog(
            body.userId || "system",
          ...
          console.error("Audit log oluşturulamadı:", err);
        }

        try {
          const mailSettings = await db
            .collection("system_settings")
            .findOne({ id: "mail_settings" });

          if (mailSettings && mailSettings.smtpHost && mailSettings.smtpUser) {
            const nodemailer = require("nodemailer");
            const envanterTip = await db
              .collection("envanter_tipleri")
              .findOne({ id: envanter.envanterTipiId });
            ...
            if (admins.length > 0) {
              ...
              for (const admin of admins) {
                try {
                  ...
                  console.error(
                    `Mail gönderilemedi (${admin.email}):`,
                    mailError,
                  );
                }
              }
              console.log(
                `Yeni envanter bildirimi gönderildi: ${admins.length} kişiye`,
              );
            }
          }
        } catch (error) {
    ...
    if (
      ...
      if (!envanter) {
        return handleCORS(
          NextResponse.json({ error: "Envanter bulunamadı" }, { status: 404 }),
        );
      }
    ...
    if (
      ...
      const currentEnvanter = await db
        .collection("envanterler")
        .findOne({ id, deletedAt: null });
      if (!currentEnvanter) {
        return handleCORS(
      ...
      if (
        body.durum &&
        body.durum !== "Zimmetli" &&
        body.durum !== currentEnvanter.durum
      ) {
        const activeZimmet = await db.collection("zimmetler").findOne({
          envanterId: id,
          durum: "Aktif",
        ...
        if (activeZimmet) {
          return handleCORS(
            NextResponse.json(
              {
                error:
                  "Cihaz zimmetliyken durum değiştirilemez. Önce iade alın.",
              },
              { status: 409 },
            ),
          );
      ...
      const { userId, userName, oldDurum, logStatusChange, ...cleanBody } =
        body;
      ...
      if (cleanBody.marka && cleanBody.marka !== currentEnvanter.marka) {
        changedFields.marka = {
          onceki: currentEnvanter.marka,
          yeni: cleanBody.marka,
        };
      }
      if (cleanBody.model && cleanBody.model !== currentEnvanter.model) {
        changedFields.model = {
          onceki: currentEnvanter.model,
          yeni: cleanBody.model,
        };
      }
      if (
        cleanBody.seriNumarasi &&
        cleanBody.seriNumarasi !== currentEnvanter.seriNumarasi
      ) {
        changedFields.seriNumarasi = {
          onceki: currentEnvanter.seriNumarasi,
          yeni: cleanBody.seriNumarasi,
        };
      }
      if (cleanBody.durum && cleanBody.durum !== currentEnvanter.durum) {
        changedFields.durum = {
          onceki: currentEnvanter.durum,
          yeni: cleanBody.durum,
        };
      }
      if (
        cleanBody.notlar !== undefined &&
        cleanBody.notlar !== currentEnvanter.notlar
      ) {
        changedFields.notlar = {
          onceki: currentEnvanter.notlar || "",
          yeni: cleanBody.notlar,
        };
      }
      if (
        cleanBody.envanterTipiId &&
        cleanBody.envanterTipiId !== currentEnvanter.envanterTipiId
      ) {
        changedFields.envanterTipiId = {
          onceki: currentEnvanter.envanterTipiId,
          yeni: cleanBody.envanterTipiId,
        };
      }
    ...
    if (route.startsWith("/envanterler/") && method === "DELETE") {
      ...
      const existingEnvanter = await db
        .collection("envanterler")
        .findOne({ id, deletedAt: null });
      ...
      const enrichedZimmetler = await Promise.all(
        zimmetler.map(async (zimmet) => {
          ...
          if (zimmet.iadeAlanYetkiliId) {
            const yetkili = await db
              .collection("calisanlar")
              .findOne({ id: zimmet.iadeAlanYetkiliId });
            if (yetkili) {
              iadeAlanYetkili = {
                id: yetkili.id,
    ...
    if (route === "/zimmetler" && method === "POST") {
      ...
      const envanter = await db
        .collection("envanterler")
        .findOne({ id: body.envanterId });
      const calisan = await db
        .collection("calisanlar")
        .findOne({ id: body.calisanId });
    ...
    if (route === "/zimmetler/iade" && method === "POST") {
      ...
      if (
        ...
        return handleCORS(
          NextResponse.json(
            {
              error:
                "Zimmet ID, iade tarihi, envanter durumu ve yetkili ID zorunludur",
            },
            { status: 400 },
          ),
        );
      ...
      if (!yetkili) {
        return handleCORS(
          NextResponse.json({ error: "Yetkili bulunamadı" }, { status: 404 }),
        );
      }
      ...
      if (!zimmet) {
        return handleCORS(
          NextResponse.json({ error: "Zimmet bulunamadı" }, { status: 404 }),
        );
      }
    ...
    if (route === "/seed" && method === "POST") {
      ...
      const calisanlar = [
        {
          id: uuidv4(),
          adSoyad: "Admin User",
          email: "admin@halktv.com.tr",
          telefon: "555-9999",
          departmanId: departmanlar[0].id,
          durum: "Aktif",
          yoneticiYetkisi: true,
          adminYetkisi: true,
          sifre: "admin123",
          createdAt: new Date(),
          updatedAt: new Date(),
      ...
      const tipler = [
        {
          id: uuidv4(),
          ad: "Laptop",
          aciklama: "Dizüstü Bilgisayarlar",
          createdAt: new Date(),
          updatedAt: new Date(),
          deletedAt: null,
        },
        ...
        },
        {
          id: uuidv4(),
          ad: "Monitör",
          aciklama: "Bilgisayar Monitörleri",
          createdAt: new Date(),
          updatedAt: new Date(),
          deletedAt: null,
        },
      ...
      const envanterler = [
        {
          id: uuidv4(),
          envanterTipiId: tipler[0].id,
          marka: "Apple",
          model: 'MacBook Pro 16"',
          seriNumarasi: "MBA123456",
          durum: "Zimmetli",
          notlar: "",
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        {
          id: uuidv4(),
          envanterTipiId: tipler[1].id,
          marka: "Apple",
          model: "iPhone 14 Pro",
          seriNumarasi: "IPH345678",
          durum: "Zimmetli",
          notlar: "",
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        {
          id: uuidv4(),
          envanterTipiId: tipler[2].id,
          marka: "LG",
          model: '27" UltraWide',
          seriNumarasi: "LGM901234",
          durum: "Depoda",
          notlar: "",
          createdAt: new Date(),
          updatedAt: new Date(),
    ...
    if (route === "/audit-logs" && method === "GET") {
      ...
      if (inventoryIds.size > 0) {
        ...
        logs.forEach((log) => {
          ...
          if (log.details && log.details.inventoryId)
            relatedInventoryId = log.details.inventoryId;
    ...
    if (
      ...
      return handleCORS(
        NextResponse.json(accessories.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (
      ...
      const result = await db
        .collection("inventory_accessories")
        .updateOne({ id: accessoryId, deletedAt: null }, { $set: updateData });

      if (result.matchedCount === 0) {
        return handleCORS(
          NextResponse.json({ error: "Aksesuar bulunamadı" }, { status: 404 }),
        );
      }
    ...
    if (route === "/dijital-varlik-kategorileri" && method === "GET") {
      const kategoriler = await db
        .collection("dijital_varlik_kategorileri")
        .find({ deletedAt: null })
        .sort({ ad: 1 })
        .toArray();

      return handleCORS(
        NextResponse.json(kategoriler.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route.startsWith("/dijital-varlik-kategorileri/") && method === "PUT") {
      ...
      if (result.matchedCount === 0) {
        return handleCORS(
          NextResponse.json({ error: "Kategori bulunamadı" }, { status: 404 }),
        );
      }
      ...
      if (usedByAsset) {
        return handleCORS(
          NextResponse.json(
            {
              error:
                "Bu kategoriye bağlı dijital varlıklar var. Önce onları silin veya kategorisini değiştirin.",
            },
            { status: 409 },
          ),
        );
    ...
    if (route === "/deleted/envanterler" && method === "GET") {
      ...
      const enriched = deleted.map((env) => ({
        ...env,
        envanterTipiAd:
          envanterTipleri.find((t) => t.id === env.envanterTipiId)?.ad ||
          "Bilinmiyor",
      }));

      return handleCORS(
        NextResponse.json(enriched.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route === "/deleted/calisanlar" && method === "GET") {
      ...
      const enriched = deleted.map((cal) => ({
        ...cal,
        departmanAd:
          departmanlar.find((d) => d.id === cal.departmanId)?.ad ||
          "Bilinmiyor",
      }));
    ...
    if (route === "/dijital-varliklar" && method === "GET") {
      const dijitalVarliklar = await db
        .collection("dijital_varliklar")
        .find({ deletedAt: null })
        .sort({ createdAt: -1 })
        .toArray();
      ...
      return handleCORS(
        NextResponse.json(enrichedData.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route.startsWith("/dijital-varliklar/") && method === "PUT") {
      ...
      const currentDijitalVarlik = await db
        .collection("dijital_varliklar")
        .findOne({ id, deletedAt: null });
      ...
      const updateData = {
        ...updateFields,
        baslangicTarihi: updateFields.baslangicTarihi
          ? new Date(updateFields.baslangicTarihi)
          : null,
        bitisTarihi: updateFields.bitisTarihi
          ? new Date(updateFields.bitisTarihi)
          : null,
        updatedAt: new Date(),
      };
      ...
      if (updateFields.ad && updateFields.ad !== currentDijitalVarlik?.ad) {
        changedFields.ad = {
          onceki: currentDijitalVarlik?.ad,
          yeni: updateFields.ad,
        };
      }
      if (
        updateFields.hesapEmail !== undefined &&
        updateFields.hesapEmail !== currentDijitalVarlik?.hesapEmail
      ) {
        changedFields.hesapEmail = {
          onceki: currentDijitalVarlik?.hesapEmail || "",
          yeni: updateFields.hesapEmail,
        };
      }
      if (
        updateFields.hesapKullaniciAdi !== undefined &&
        updateFields.hesapKullaniciAdi !==
          currentDijitalVarlik?.hesapKullaniciAdi
      ) {
        changedFields.hesapKullaniciAdi = {
          onceki: currentDijitalVarlik?.hesapKullaniciAdi || "",
          yeni: updateFields.hesapKullaniciAdi,
        };
      }
      if (
        updateFields.durum &&
        updateFields.durum !== currentDijitalVarlik?.durum
      ) {
        changedFields.durum = {
          onceki: currentDijitalVarlik?.durum,
          yeni: updateFields.durum,
        };
      }
      if (
        updateFields.lisansTipi &&
        updateFields.lisansTipi !== currentDijitalVarlik?.lisansTipi
      ) {
        changedFields.lisansTipi = {
          onceki: currentDijitalVarlik?.lisansTipi,
          yeni: updateFields.lisansTipi,
        };
      }
      if (
        updateFields.calisanId !== undefined &&
        updateFields.calisanId !== currentDijitalVarlik?.calisanId
      ) {
        changedFields.calisanId = {
          onceki: currentDijitalVarlik?.calisanId,
          yeni: updateFields.calisanId,
        };
      }
      if (
        updateFields.envanterId !== undefined &&
        updateFields.envanterId !== currentDijitalVarlik?.envanterId
      ) {
        changedFields.envanterId = {
          onceki: currentDijitalVarlik?.envanterId,
          yeni: updateFields.envanterId,
        };
      }
    ...
    if (route === "/amortisman-raporu" && method === "GET") {
      ...
      const enrichedEnvanterler = await Promise.all(
        envanterler.map(async (envanter) => {
          ...
          const alisTarihi = envanter.alisTarihi
            ? new Date(envanter.alisTarihi)
            : null;
          ...
          if (alisTarihi && alisFiyati > 0) {
            gecenYil =
              (today.getTime() - alisTarihi.getTime()) /
              (1000 * 60 * 60 * 24 * 365);
            yillikAmortisman = alisFiyati * (amortismanOrani / 100);
            birikimliAmortisman = Math.min(
              yillikAmortisman * gecenYil,
              alisFiyati,
            );
    ...
    if (route === "/bakim-kayitlari" && method === "GET") {
      ...
      return handleCORS(
        NextResponse.json(enrichedKayitlar.map(({ _id, ...rest }) => rest)),
      );
    }
    ...
    if (route === "/bakim-kayitlari" && method === "POST") {
      ...
      const kayit = {
        ...
        aciklama: body.aciklama || "",
        bildirilenTarih: body.bildirilenTarih
          ? new Date(body.bildirilenTarih)
          : new Date(),
    ...
    if (route.startsWith("/bakim-kayitlari/") && method === "PUT") {
      ...
      // Handle date fields
      if (updateData.bildirilenTarih)
        updateData.bildirilenTarih = new Date(updateData.bildirilenTarih);
      if (updateData.baslangicTarihi)
        updateData.baslangicTarihi = new Date(updateData.baslangicTarihi);
      if (updateData.bitisTarihi)
        updateData.bitisTarihi = new Date(updateData.bitisTarihi);
      ...
      if (
        updateFields.durum === "Tamamlandı" &&
        updateFields.restoreEnvanterDurum
      ) {
        const kayit = await db.collection("bakim_kayitlari").findOne({ id });
        if (kayit) {
          await db
      ...
      if (updateFields.durum === "Tamamlandı") {
        const kayit = await db.collection("bakim_kayitlari").findOne({ id });
        const envanter = kayit
          ? await db.collection("envanterler").findOne({ id: kayit.envanterId })
          : null;
        ...
        if (mailSettings && mailSettings.smtpHost && mailSettings.smtpUser) {
          try {
            ...
            if (admins.length > 0) {
              ...
              const envanterAd = envanter
                ? `${envanter.marka} ${envanter.model}`
                : "Bilinmeyen Envanter";
              ...
              console.log(
                `Bakım bildirimi gönderildi: ${admins.length} kişiye`,
              );
            }
          } catch (error) {
    ...
    if (route === "/bakim-istatistikleri" && method === "GET") {
      ...
      const stats = {
        ...
        toplamMaliyet: {
          TRY: kayitlar
            .filter((k) => k.paraBirimi === "TRY")
            .reduce((sum, k) => sum + (k.maliyet || 0), 0),
          USD: kayitlar
            .filter((k) => k.paraBirimi === "USD")
            .reduce((sum, k) => sum + (k.maliyet || 0), 0),
          EUR: kayitlar
            .filter((k) => k.paraBirimi === "EUR")
            .reduce((sum, k) => sum + (k.maliyet || 0), 0),
          GBP: kayitlar
            .filter((k) => k.paraBirimi === "GBP")
            .reduce((sum, k) => sum + (k.maliyet || 0), 0),
        },
        arizaTiplerineGore: {
      ...
      if (!settings || !settings.smtpHost || !settings.smtpUser) {
        return handleCORS(
          NextResponse.json(
            {
              error:
                "Mail ayarları yapılandırılmamış. Önce SMTP ayarlarını kaydedin.",
            },
            { status: 400 },
          ),
        );
    ...
    if (route === "/backup/import" && method === "POST") {
      ...
      try {
        ...
        const collections = [
          ...
          {
            name: "dijital_varlik_kategorileri",
